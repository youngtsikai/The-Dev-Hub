...
function Dashboard() {
  ...
  const isNewUser = user.total_commits === 0;
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="dashboard-hero">
        <div className="hero-text">
          ...
          <p>
            ...
            >
              {user.first_name || user.github_username || user.username}
            </span>
      ...
      {isNewUser ? (
        // --- THE "WELCOME" EMPTY STATE (For first time users)---
        <div className="empty-state-container">
          <FaRocket className="empty-state-icon" />
          <h2 style={{ fontSize: "2rem", marginBottom: "10px" }}>
            Let's Get You Started!
          </h2>
          <p style={{ color: "#8b949e", maxWidth: "500px" }}>
            Your dashboard is looking a little empty. Connect your GitHub
            account to pull your commits, calculate your XP, and reveal your
            developer rank.
          </p>

          <button
            onClick={() => setShowGithubModal(true)}
            className="btn-sync-hero"
          >
            <FaGithub size={24} /> Sync GitHub Now
          </button>
        </div>
      ) : (
        // --- THE NORMAL DASHBOARD (Hidden until user synsc) ---
        <>
          ...
          <div className="dashboard-split">
            <div className="chart-section" aria-label="Activity Velocity Chart">
              ...
              >
                {(user.velocity || [0, 0, 0, 0, 0, 0, 0]).map(
                  (count, index) => (
                    <div
                      key={index}
                      className="bar"
